...
class Field:
    ...
    def __repr__(self) -> str:
        return f"{self._name}:{self._type}={self._value}" # f"{type(self)} {self._name}:{self._type}={self._value}"
...
class Referenceable(Field):
    ...
    def show(self, indent=0):
        return (' '*indent) + str(self._referred) + (('\n' + self._referred.show(indent + 2)) if self._referred else '')
...
class Storable(ABC, metaclass=StorableMeta):
        ...
        for uniqueFieldName in self._uniqueFields:
            ...
            if (field:=self.__getattribute__(uniqueFieldName)) is not None:
                if field._value is not None:
                    conditions.append(wrap_condition(col_prefix + (field.col_name() or dbfy(uniqueFieldName)), '=', field._value))
        ...
        for fieldName in self._fields:
            if fieldName not in self._uniqueFields and fieldName not in self._referenceables:
                ...
                if (field:=self.__getattribute__(fieldName)) is not None:
                    if field._value is not None:
                        conditions.append(wrap_condition(col_prefix + (field.col_name() or dbfy(fieldName)), '=', field._value))
                ...
                for field_name in self.__getattribute__(uniqueConstraint)._field_names:
                    ...
                    if field is not None:
                        try: # assume field instance
                            if field._value:
                                conditions.append(wrap_condition(col_prefix + (field.dbfy_name(field_name) or field._name), '=', field._value))
                        except AttributeError:
                            if field:
                                conditions.append(wrap_condition(col_prefix + (field_name or dbfy(field_name)), '=', field))
                    else:
                        conditions = []
                        break
            ...
            for refName in self._referenceables:
                ...
                if referenceable is not None:
                    ...
                    if referenceable._referred:
                        ...
                        ref_conditions, ref_joins = referenceable._referred.derive_conditions_and_joins(consider_joins, inspected, dbfy(refName)+".")
                        joins.append(join)
                    ...
                    if join:
                        ...
                        if join.joined_cls._fields: # !! May be false if no instance of the joined_class has been created yet
                            join.joined_col_names = [f"{src_col_name}.{dbfy(name) + ('_id' if name in join.joined_cls._referenceables else '')}"
                                 f" AS {dbfy(refName)}{STORABLE_TABLE_COLUMN_SEP}{dbfy(name) + ('_id' if name in join.joined_cls._referenceables else '')}"
    ...
    def fill(self, attr) -> "Storable":
        ...
        if SqlStore()._debug: print(f"...... Filling new {type(self)} from {attr}")
    ...
    def load_all(self, condition = '', ordering:list[tuple[str,str]] = [], consider_joins:bool = True):
        ...
        attrs_list = self._load(condition, ordering, consider_joins=consider_joins)
        result = []
        for attrs in attrs_list:
            o = copy.deepcopy(self)
            result.append(o.fill(attrs))
            o.show(0)
        return result, attrs
    ...
    def show(self, indent=0) -> str:
        return f"{type(self)}:" + ("\n"+(" "*indent)).join( map(lambda x: self.__getattribute__(x).show(indent+2), self._fields) )
